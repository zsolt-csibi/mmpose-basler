...
def main():
    ...
    video = pylon.InstantCamera(pylon.TlFactory.GetInstance().CreateFirstDevice())

    video.StartGrabbing(pylon.GrabStrategy_LatestImageOnly) 
    ...
    frame_idx = 0
    i = 0
    print(video.AcquisitionFrameRate.GetValue())
    fps = video.AcquisitionFrameRate.GetValue()
    with open(args.pred_save_path, 'w') as f:
        while video.IsGrabbing() and i < 25:
            grabResult = video.RetrieveResult(5000, pylon.TimeoutHandling_ThrowException)
